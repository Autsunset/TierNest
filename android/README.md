...
**0.2.0-alpha09** · Android 8.0+ · Kotlin / Jetpack Compose · EasyTier 2.6.4
...
热点 Wi-Fi 只提供网络接入，不会把客户端自动注册成 EasyTier 节点。

- **设备能装客户端**：在每台设备安装 EasyTier；Android 可用 TierNest 的 VPN 模式。
  填写相同网络名称、密钥和可达的接入节点，每台使用 DHCP 或不同的虚拟 IPv4。
  不能直接复制手机的静态虚拟地址。此方式每台设备都有独立节点身份。
- **设备不能装客户端**：需要手机充当子网网关。当前独立 App 尚未实现热点转发。
  模块版的「热点设备单向访问」可让热点客户端主动访问组网目标，但它们不会成为
  独立节点，远端也不能通过此选项主动访问它们。改用模块时先停止 App 核心，避免
  两个核心同时管理路由。需要双向访问时，另行配置并验证子网发布、转发和防火墙。

若手机自身需要保持组网连接或承担网关，关闭「锁屏暂停」；升级不会代替用户关闭它。
参见 [EasyTier 快速组网](https://easytier.rs/guide/network/quick-networking) 与
[子网代理说明](https://easytier.rs/guide/gui/subnet_proxy.html)。
...
输出 `dist/TierNest-App-v0.2.0-alpha09.apk`。上游下载、Cargo 依赖、工具版本和
