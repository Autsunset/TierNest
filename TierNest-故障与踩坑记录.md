...
## 2026-09-21：Root 通信超时后直接停止，缺少一次恢复机会

alpha08 的导出日志记录到两次 Root 命令超时，随后服务主动清理连接并保存
requested=false；本轮记录没有对应的新 FATAL 或 App 重启，不能把这两次断线
等同于新的闪退。历史系统崩溃摘要缺少堆栈，仍不足以确定旧闪退根因。另有多次
正常锁屏暂停，属于已启用的用户设置；修复不会自动关闭它。

原 status/peers/sync 的截止时间与响应处理继承主线程，错误处理又在 operations
锁外清理，可能与界面采样交错。alpha09 将完整命令协议移至 IO；控制循环和采样
共用持锁的错误处理，清理前立即撤销 coreActive。采样直接传递原始超时，不再等到
下一次检查才把它降格为通用的“核心退出”。生命周期收尾也等待同一操作锁。

已成功完成启动与路由同步的 Root 连接，通信超时后最多自动恢复一次：关闭旧会话、
检查客户端是否退出、等待 1.5 秒，重新按最新请求、模式、锁屏和家庭网络决定是否
启动。新会话仍必须通过既有所有者锁与残留路由清理检查。清理异常保留在原异常的
suppressed 链中并阻止自动重试。预算按服务生命周期计算，网络回调、配置刷新和
重连成功不补充预算；首次启动失败、非超时或 VPN 错误不会无限尝试。
手动停止在任何阶段都优先；清理期间的新请求不会被旧失败覆盖。预算只在实际
重新启动核心时消耗，退避期间进入锁屏或家庭待机会撤销预约，不消耗重试机会。没有新增唤醒锁、周期守护或待机轮询。

诊断增加固定枚举阶段与数值时间，不记录请求、响应正文或网络身份。记录首行是
本地消费时间，并非远端生成时间；分别保留含深睡眠/不含深睡眠耗时。移到 IO
不能消除阻塞写入、全进程暂停或调度延迟，25 秒仍不是可保证的硬上界。

故障注入覆盖无响应、半截响应、写入失败、无效结束帧、完整操作失败后继续调用、
父取消、主调用线程阻塞时 IO 截止时间仍能触发，以及手动停止、待机、清理失败、
新请求、单次预算与服务重建。实体 arm64 手机后台、Root 管理器和长期待机仍需
实机观察，不能用 JVM 测试替代。

本地 67 项 JVM、16 项 Root 脚本、4 项 APK 隐私回归及 release Lint 均通过。
云测试机覆盖安装后原偏好字节一致，最终包替换测试配置时 TOML/偏好也字节一致；
Root 权限不足连续两次启动均显示错误和“重新连接”，每次只对应一次用户点击，
没有自动循环。测试后恢复并逐文件核对原应用数据，删除合成配置与测试日志。
此环境不支持本 App 的真实 arm64 Root 连接，不能代表实体手机超时恢复或待机验收。


## 2026-09-21：独立 App 的单向热点共享

alpha10 增加默认关闭的 Root 热点访问。客户端无需安装组网软件，通过手机源地址
转换访问已接管的虚拟网段和远端子网。普通 VPN 模式保留偏好但不调用 Root 共享。
原 TOML 不自动增加 proxy_network，不把热点客户端伪装成独立节点。

Android 的热点下游接口经常不在 ConnectivityManager.allNetworks 中；若只沿用
上联网段排除，远端通告与热点相同的网段时可能被错误写入 overlay 表。App 现在
把实际 Wi-Fi 接口地址一并加入物理网段排除，Root 应用共享前再次核对不重叠。
热点识别只读取 dumpsys 的当前 Tether state 段，要求 TetheredState/lastError=0、
接口 UP、私网 IPv4 且不是默认上联，历史日志中的旧状态不算当前热点。读取不到
明确状态时显示错误，不因 wlan 名称猜测热点。

Android 的策略路由可能没有 main 表兜底；只有 MASQUERADE/FORWARD 规则不能保证
回复回到热点。因此添加独立返回表，并用 iif=tiernest0、to=热点网段的精确规则
限定查表范围。FORWARD 只接受客户端到已接管目标，以及反向 ESTABLISHED/RELATED，
拒绝远端新建连接。若本机发布了热点子网，用户态代理可能绕过 FORWARD，因此拒绝
启用单向共享并提示冲突，不静默改写配置或作出错误的单向访问承诺。

所有命令在现有 Root 会话锁中执行，清理记录在变更前写入。只删除完整匹配的
规则和自有协议/接口路由，不按优先级单独删除，不刷新整张表或整条链。同名外来
链拒绝占用，外来规则保留；清理失败保留记录并报告，不能假装成功。进程重建先
清理上次残留，再启动核心。没有新模块包或常驻工作线程。

维护检查采用两条自有链快照与三个入口检查，避免每 60 秒按目标数执行数百次
iptables。系统热点事件只触发重新读取，广播内容不会直接作为接口或路由参数。
关闭共享不关闭系统热点，手动停止和锁屏/家庭待机仍优先；需要持续共享须自行
关闭锁屏暂停，升级不会代替用户改动此选择。

Android 15 模拟器隔离网络实测：客户端访问组网、源地址转换、专用返回路由、普通
热点上网不变、拒绝远端主动访问、重复维护幂等、关闭后完整恢复基线路由/防火墙、
新进程通过记录清理均通过。云测试机的隔离命名空间缺少 legacy iptables filter
支持，不能在那里验证实际转发；实体热点/硬件加速/厂商防火墙仍待实机确认。

本地通过 20 项热点状态回归（含外来对象替换、删除失败保留记录和旧模块残留拒绝）、
16 项已有 Root 回归、70 项 JVM、4 项隐私测试和 release Lint。设置页实测开关默认
关闭，开启后持久化，重启 App 和切换 VPN 保留选择与手动停止，且没有调用 Root；
覆盖安装原偏好字节一致。模拟器原配置与偏好已逐文件恢复并同步落盘。


## 2026-09-21：正式版候选的签名、配置事务与运行时收敛

以 alpha10 为基线冻结新功能。发布私钥从 SDK 默认 debug 配置迁入仓库外独立密钥库，
证书身份保持不变以支持覆盖升级；CI 使用独立包名、应用名和测试密钥。发布检查核对
实际 APK 的包名、版本、签名、非调试标志、双 ABI、ELF/ZIP 的 16 KiB 对齐、完整
Root 二进制清单和隐私。CI 不接触维护者私钥，错误模式或签名的包无法通过检查。

Android 8/9 的 Download 备份原先仅报缺少存储权限，需要用户自行到系统设置授权。
候选版按需请求权限，允许后继续原操作，拒绝时保留配置与草稿。实测覆盖直接备份和
保存触发自动备份；配置保存成功前核对旧文件备份字节。缺少 DocumentsUI 或浏览器
时显示提示，避免启动外部界面抛异常；家庭网络列表显示复验错误，文案与热点实际能力一致。

模块导入过去先替换 TOML 再提交审阅标记，两个文件之间失败可能失去迁移保护。
现在先写入停止状态与非空审阅信息，再写 TOML；配置写入失败主动恢复原文，确认
配置恢复后再回滚导入偏好。回滚失败保留审阅限制和原异常证据，始终不恢复 requested=true，
不覆盖当时最新的外观、锁屏和热点选择。提交前重新通过服务发出停止请求，覆盖快照
复制期间来自磁贴的新连接请求。AtomicFile 的部分失败只记录日志，因此写配置另加
显式 fd.sync 与重新读取核对，不能仅凭 finishWrite 返回就宣称写入成功。

Root 校验/备份的完整错误响应不代表管道损坏。原通用命令收尾在这些错误下也关闭
活跃会话，可能使配置操作误伤组网。只读操作现在保留完整响应后的健康连接，临时会话
仍关闭；传输错误、超时和取消仍清理。无效草稿先在进入 Root 会话前本地拒绝。

热点清理直接尝试按完整身份删除规则，失败才查询。必须区分不存在码与查询/锁/权限
错误；仅可读表里的明确不存在可作为已清理。测试模型同时检查自身 Traceback，避免
mock 崩溃伪装成权限失败。128 目标的同一 Android 15 隔离网络单次测量，清理由约
1.91 秒降至 1.00 秒；调用数下降是确定事实，耗时不代表实体设备或硬截止上限。

VPN 对已有覆盖内的冗余路由去重，避免同网段节点变化引起不必要的系统接口重建。
不合并相邻独立网段，不扩大接管范围；物理网络冲突仍在此前排除。原始 TOML 的 VPN MTU
须满足已有范围，原生核心与系统接口保持一致，不再对一侧静默截断。

连续运行测试同时记录 HTTP 数据、进程、FD、线程与 RSS。首轮 30 分钟没有进程重启或
资源持续增长，但出现 3 次采样异常；原工具没有保存超时阶段和已收到数据，不能把它们
直接归因于应用断线。补充连接/空闲/EOF 超时及阶段记录后再复测，保留原始异常统计。
实体 arm64 Root 管理器、真实热点/移动网络/Clash 共存及长期锁屏仍需单独验收。

追加 100 次快速请求全部成功；补全超时记录后的 5 分钟复测为 61/61 成功、无工具
超时，PID 不变。首轮 30 分钟的 3 次异常仍保留，未被确认为应用断线或宣称已彻底解决。
维护者包覆盖升级保留原 TOML 与偏好字节，正常启动追加诊断日志和更新 profile 缓存。
本轮保持 RC 标记，等待实体设备及更长时间的验证。
